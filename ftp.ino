// File: ftp.ino for ESP8266 NodeMCU
/*
Lagodish inc.
*/
...
const char *ssid = "Room_105"; //WIFI DATA CHANGE!!!!!!!!!!!!!!!!!!!!!!
const char *password = "12345678"; //WIFI PASS CHANGE!!!!!!!!!!!!!!!!!!!
...
IPAddress server( 192, 168, 31, 13 );  // SERVER FTP ADRESS CHANGE!!!!!!!!!!!!!!!!!!!!!!
...
byte doFTP(boolean upload) {
  ...
  if (upload) {
    ...
    fh.println(MetaData + String(schet));
    schet++;
  ...
  client.println(F("USER pi"));   // USER CHANGE (pi) !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  if (!eRcv()) return 0;
  if (debug) Serial.println("Send PASSWORD");
  
  client.println(F("PASS 12345678")); //PASS (12345678) CHANGE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  if (!eRcv()) return 0;
  ...
  if (upload) {
    ...
    Serial.println("Send");
    client.print(F("STOR "));
    client.println("/home/pi/"+fileName);  //FTP HOME DIR CHANGE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  } else {
...
void loop() {
  byte inChar;
  timer ++;
  if(timer > 6000){    // AUTO UPLOAD 1MIN!  
    inChar = 'u';
    timer = 0;
    }
    else
